...
int main()
{
  ...
  {
  } // checksum (i.e cs array ) should have all zeroes after performing crc operation, so if any 1 is found e is not incremented and hence e will become less than N-1 and therefore we can say there is an error in the transmitted message.
  if (e < N - 1)
    printf("\nError detected\n\n");
  else
    printf("\nNo error detected\n\n");
